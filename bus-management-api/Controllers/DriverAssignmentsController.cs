...
public class DriverAssignmentsController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateAssignmentDto dto)
    {
        ...
        await _context.SaveChangesAsync(); // Save first so assignment.AssignmentId is populated

        // Prevent duplicate: skip if same assignment notification already exists for this driver
        var alreadyNotified = await _context.Notifications.AnyAsync(n =>
            n.UserId == driver.UserId &&
            n.Type == "Assignment" &&
            n.RelatedEntityId == assignment.AssignmentId);
    ...
    public async Task<IActionResult> Respond(int id, [FromBody] dynamic payload)
    {
        ...
        // Notify Admins — skip if already notified for this assignment response
        var admins = await _context.Users.Where(u => u.UserType == "Admin" && u.IsActive).ToListAsync();
        var alreadyNotifiedAdminIds = await _context.Notifications
            .Where(n => n.Type == "Alert" && n.RelatedEntityId == assignment.AssignmentId && n.Title == $"Assignment {response}")
            .Select(n => n.UserId)
            .ToListAsync();
        ...
        {
            if (alreadyNotifiedAdminIds.Contains(admin.UserId)) continue;
