...
public class SchedulesController : ControllerBase
{
    ...
    public async Task<IActionResult> Create([FromBody] CreateScheduleDto dto)
    {
        ...
        // Prevent double-booking: bus cannot have overlapping schedules
        var hasOverlap = await _context.Schedules.AnyAsync(s =>
            s.BusId == dto.BusId &&
            s.DepartureTime < dto.ArrivalTime &&
            s.ArrivalTime > dto.DepartureTime);
        if (hasOverlap)
            return BadRequest(ApiResponse<ScheduleDto>.Fail(
                "This bus already has a schedule during that time. Choose a different bus or adjust the time."));
    ...
    public async Task<IActionResult> Update(int id, [FromBody] CreateScheduleDto dto)
    {
        ...
        // Prevent double-booking on update (exclude current schedule)
        var hasUpdateOverlap = await _context.Schedules.AnyAsync(s =>
            s.ScheduleId != id &&
            s.BusId == dto.BusId &&
            s.DepartureTime < dto.ArrivalTime &&
            s.ArrivalTime > dto.DepartureTime);
    ...
    public async Task<IActionResult> Delete(int id)
    {
        ...
        // Cascade: remove tickets before deleting schedule
        var tickets = _context.Tickets.Where(t => t.ScheduleId == id);
