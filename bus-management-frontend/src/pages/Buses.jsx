...
import { Plus, Edit2, Trash2, Bus as BusIcon, Search, Upload, ToggleLeft, ToggleRight, Wrench, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function Buses() {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            if (editingBus) {
                await busService.update(editingBus.busId, { ...data, agencyId: data.agencyId ? parseInt(data.agencyId) : null, status: data.status || 'Active' });
            } else {
                await busService.create({ ...data, agencyId: data.agencyId ? parseInt(data.agencyId) : null, status: 'Active' });
            }
            fetchBuses();
            closeModal();
    ...
    const toggleStatus = async (bus) => {
        ...
        const currentIdx = statuses.indexOf(bus.status || 'Active');
        const nextStatus = statuses[(currentIdx + 1) % statuses.length];
        try {
            await busService.update(bus.busId, {
            ...
            toast.success(`Bus ${bus.busNumber} → ${nextStatus}`);
            fetchBuses();
        } catch {
        ...
        <div className="space-y-6 animate-fade-in-up">
                                    ...
                                    <tr key={bus.busId} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <button
                                                ...
                                                title="Click to cycle: Active → Inactive → Maintenance"
                                                className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-all hover:scale-105 cursor-pointer border
                                            ...
                                            >
                                                <div className={`w-1.5 h-1.5 rounded-full ${
                                                    bus.status === 'Active' || !bus.status ? 'bg-emerald-500' :
                                                    bus.status === 'Maintenance' ? 'bg-amber-500' : 'bg-rose-500'
                                                }`} />
                                                {bus.status === 'Maintenance' ? <Wrench size={11} /> : null}
                                                {bus.status || 'Active'}
                                            </button>
                                        </td>
            ...
            <Modal isOpen={isModalOpen} onClose={closeModal} title={editingBus ? 'Edit Fleet Vehicle' : 'Add New Vehicle'}>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    ...
                    {editingBus && (
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">Bus Status</label>
                            <select
                                {...register('status')}
                                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:bg-white focus:ring-4 focus:ring-nexus-500/10 focus:border-nexus-500 transition-all outline-none text-slate-900"
                            >
                            ...
                            <p className="text-xs text-slate-400 mt-1">Passengers cannot book Inactive or Maintenance buses.</p>
                        </div>
                    )}
