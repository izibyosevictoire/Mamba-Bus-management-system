...
export default function Schedules() {
    ...
    const onSubmit = async (data) => {
        ...
        const busId = parseInt(data.busId);
        const conflict = schedules.find(s =>
            s.busId === busId &&
            (!editingSchedule || s.scheduleId !== editingSchedule.scheduleId) &&
            new Date(s.departureTime) < arrival &&
            new Date(s.arrivalTime) > departure
        );
        if (conflict) {
            const r = getRoute(conflict.routeId);
            toast.error(`Bus conflict: this bus is already scheduled on ${r ? `${r.origin} → ${r.destination}` : 'another route'} from ${format(parseLocalDate(conflict.departureTime), 'p')} to ${format(parseLocalDate(conflict.arrivalTime), 'p')} on ${format(parseLocalDate(conflict.departureTime), 'dd MMM yyyy')}.`);
            return;
        }
    ...
    return (
        <div className="space-y-6 animate-fade-in-up">
            <div className="flex flex-col items-start justify-between gap-4 mb-8 sm:flex-row sm:items-center">
                <div>
                    <h1 className="text-3xl font-medium tracking-tight font-display text-slate-900">Schedule Management</h1>
                    <p className="mt-1 text-slate-500">Plan and coordinate active transit routes and timings.</p>
                </div>
                <button
...
<span>Create Schedule</span>                                                        
            ...
            <div className="overflow-hidden glass-card rounded-2xl">
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="text-xs font-semibold tracking-wider uppercase border-b bg-slate-50/80 text-slate-500 border-slate-100">
                            <tr>
                        ...
                        <tbody className="divide-y divide-slate-100/60">
                            {loading ? (
                                <tr><td colSpan={6} className="py-16 text-center text-slate-400">
                                    <Loader2 className="w-6 h-6 mx-auto mb-2 animate-spin text-nexus-500" />
                                    Loading schedules...
                                </td></tr>
                            ) : schedules.length === 0 ? (
                                <tr><td colSpan={6} className="py-16 text-center text-slate-400">
                                ...
                                return (
                                    <tr key={schedule.scheduleId} className="transition-colors hover:bg-slate-50/80 group">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className="px-2 py-1 text-xs font-semibold rounded text-nexus-600 bg-nexus-50">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                <button
                                                    ...
                                                    className="p-2 transition-all text-slate-400 hover:text-nexus-600 hover:bg-nexus-50 rounded-xl"
                                                    title="Edit"
                                                >
                                                    <Edit2 size={17} />
                                                </button>
                                                <button
                                                    ...
                                                    className="p-2 transition-all text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl"
                                                    title="Delete"
                                                >
                                                    <Trash2 size={17} />
                                                </button>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                    <div>
                        ...
                        <select
                            ...
                            className="w-full px-4 py-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-nexus-500/30 focus:border-nexus-500 text-slate-900"
                        >
                        ...
                        {errors.routeId && <p className="mt-1 text-xs text-rose-500">{errors.routeId.message}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        {errors.busId && <p className="mt-1 text-xs text-rose-500">{errors.busId.message}</p>}
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-nexus-500/30 focus:border-nexus-500 text-slate-900"
                            />
                            {errors.departureTime && <p className="mt-1 text-xs text-rose-500">{errors.departureTime.message}</p>}
                        </div>
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-3 border outline-none bg-slate-50 border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-nexus-500/30 focus:border-nexus-500 text-slate-900"
                            />
                            {errors.arrivalTime && <p className="mt-1 text-xs text-rose-500">{errors.arrivalTime.message}</p>}
                        </div>
                    </div>

                    <div className="flex items-start gap-2 p-3 text-xs border bg-amber-50 border-amber-200 rounded-xl text-amber-700">
            ...
            <Modal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Schedule">
                <div className="flex flex-col items-center gap-4 py-2 text-center">
                    <div className="flex items-center justify-center rounded-full w-14 h-14 bg-rose-100">
                        <AlertTriangle size={26} className="text-rose-600" />
                    </div>
                    <div>
                        <p className="text-lg font-semibold text-slate-900">Delete this schedule?</p>
                        {deleteTarget && (
                        ...
                        <p className="mt-2 text-xs text-rose-500">This will also delete all tickets for this schedule.</p>
                    </div>
                </div>
