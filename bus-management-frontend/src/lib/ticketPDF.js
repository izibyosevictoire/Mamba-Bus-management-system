...
export const generateTicketPDF = async (ticket) => {
    ...
    const statusVal = isPaid ? 'CONFIRMED \u2713' : (ticket.paymentStatus || ticket.status || 'PENDING');

    drawCell('Departure', depTime, cols[0], gridY + cellH + 1);
    drawCell('Date', depDate, cols[1], gridY + cellH + 1);
    drawCell('Status', statusVal, cols[2], gridY + cellH + 1, isPaid);
    ...
    const infoItems = [
        ...
        { label: 'CLASS:', value: 'Economy', color: textDark },
